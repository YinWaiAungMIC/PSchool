...
@section('content')
    <div class="roles">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Edit Timetable</h2>
            </div>
        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('timetable.update',$timetable->id) }}" method="POST" class="w-full max-w-xl px-6 py-12">
                @csrf
                @method('PUT')
                            ...
                            <select name="class_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                ...
                                @foreach ($classes as $class)
                                    <option value="{{ $class->id }}"
                                        {{ ($class->id === $timetable->class_id) ? 'selected' : '' }}
                                    >
                                        {{ $class->class_name }}
                                    </option>
                                @endforeach
